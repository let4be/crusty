...
impl Crusty {
	...
	fn metrics_queue_handler(&mut self) -> Sender<Vec<QueueMeasurement>> {
		...
		let (tx, rx) =
			bounded_ch::<Vec<QueueMeasurement>>(cfg.concurrency_profile.transit_buffer_size());
	...
	fn metrics_db_handler(&mut self) -> Sender<Vec<chu::GenericNotification>> {
		let state = self.state.clone();
		let cfg = self.cfg.clone();
		let (tx, rx) = bounded_ch::<Vec<chu::GenericNotification>>(
			cfg.concurrency_profile.transit_buffer_size(),
		);
		self.spawn(TracingTask::new(span!(), async move {
	...
	fn metrics_task_handler(&mut self) -> Sender<Vec<TaskMeasurement>> {
		...
		let cfg = self.cfg.clone();
		let (tx, rx) =
			bounded_ch::<Vec<TaskMeasurement>>(cfg.concurrency_profile.transit_buffer_size());
		self.spawn(TracingTask::new(span!(), async move {
	...
	fn domain_insert_handler(
		&mut self,
		tx_notify: Sender<chu::Notification<Domain>>,
	) -> Sender<Vec<Domain>> {
		let state = self.state.clone();
		let cfg = self.cfg.clone();
	...
	fn add_queue_measurement<F: Fn() -> usize + Send + Sync + 'static>(
		&mut self,
		kind: QueueKind,
		len_getter: F,
	) {
	...
	fn job_reader(
		...
		TracingTask::new(span!(), async move {
			...
			job_reader
				.go(state.client.clone(), tx_job, rx_sig, tx_metrics_db, rx_domain_update_notify_p)
				.await?;
			Ok(())
		})
	}
	...
	fn go(mut self) -> PinnedFut<'static, ()> {
		TracingTask::new(span!(), async move {
			...
			let (crawler, tx_job, rx_job_state_update) = crusty_core::MultiCrawler::new(
				&pp,
				self.cfg.concurrency_profile.clone(),
				network_profile,
			);
			...
			let (tx_domain_insert_notify, rx_domain_insert_notify) =
				bounded_ch::<chu::Notification<Domain>>(
					self.cfg.concurrency_profile.transit_buffer_size(),
				);
...

			{
				...
				self.add_queue_measurement(QueueKind::DomainInsertNotify, move || {
					tx_domain_insert_notify.len()
				});
				self.add_queue_measurement(QueueKind::DomainUpdateNotify, move || {
					tx_domain_update_notify.len()
				});
				self.add_queue_measurement(QueueKind::DomainUpdateNotifyP, move || {
					tx_domain_update_notify_p.len()
				});
			...
			self.spawn(Crusty::monitor_queues(
				self.state.clone(),
				self.cfg.clone(),
				rx_sig,
				tx_metrics_queue,
